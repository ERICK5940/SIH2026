...
export default function AffectedVehicles({ liveVehicles }: { liveVehicles: Record<string,any> }) {
  ...
  if(!last) return <div className="bg-white border rounded-lg p-4"><h3 className="text-xs font-black tracking-widest">{t("AFFECTED VEHICLES")}</h3><p className="text-xs text-slate-500 mt-2">No incidents yet</p></div>;
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-4">
      <h3 className="text-xs font-black tracking-widest">{t("AFFECTED VEHICLES")} • {affected.length} / {Object.keys(liveVehicles).length}</h3>
