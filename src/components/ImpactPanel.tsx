...
export default function ImpactPanel({ incidents, liveVehicles }: { incidents: any[]; liveVehicles: Record<string, any> }) {
  ...
  if (!mounted) return <div className="bg-white border border-slate-200 rounded-lg p-3" suppressHydrationWarning><h3 className="text-xs font-black tracking-widest">{t("IMPACT ANALYSIS • TURF BUFFER 80KM")}</h3><p className="text-[11px] text-slate-500 mt-1">Loading impact…</p></div>;
