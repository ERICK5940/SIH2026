          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {(() => { const order=["reported","verified","assigned","response","resolved"]; const cur=incident.lifecycle||"reported"; const idx=order.indexOf(cur); const next=order[idx+1]; if(!next) return <p className="text-[11px] font-black text-emerald-600 mt-1">✓ Resolved</p>; return <button onClick={async()=>{ await fetch("/api/incidents",{method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({id: incident.id, lifecycle: next})}); try{ const r=await fetch("/api/incidents",{cache:"no-store"}); const j=await r.json(); if(j.incidents) { const el=document.querySelector("[data-incidents]"); if(el) window.dispatchEvent(new CustomEvent("incidents-updated",{detail: j.incidents})); } }catch{} }} className="mt-2 px-2 py-1 rounded bg-sky-600 text-white text-[11px] font-black">→ {next}</button>; })()}
              </div>
