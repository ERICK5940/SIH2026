...
export default function FieldPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    // Stable ID for idempotency — crypto.randomUUID is collision-free, fallback to Date.now+random
    const stableId = typeof crypto !== "undefined" && (crypto as any).randomUUID ? (crypto as any).randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const report = { id: stableId, type, description: `${authority} (${role}): ${desc}`, photoUrl: photo, location: { latitude: lat, longitude: lng }, severity, accessibilityStatus: access, timestamp: new Date().toISOString(), offline: !navigator.onLine, authority, role, state: stateName, district: districtName, road: roadName, lifecycle: "reported" };
    try {
      const r = await fetch("/api/incidents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(report) });
      if (r.ok) {
        // Only show success when Supabase confirmed persistence
        alert("Report saved successfully.");
        setDesc(""); setPhoto(null);
        // Trigger dashboard refresh via polling — no manual reload needed
      } else {
        const err = await r.json().catch(() => ({}));
        console.error("Incident persist failed:", err);
        // Do NOT queue on server validation error (400) — show error
        if (r.status >= 400 && r.status < 500) {
          alert(`Failed to save report: ${err.error || r.statusText}`);
        } else {
          // 500 server error — treat as transient, queue for retry
          await queueReport(report); setQueuedCount((await getQueued()).length);
          alert("Report saved offline and queued for synchronization.");
        }
      }
    } catch (e: any) {
      // Network/offline error — queue idempotently; same id will upsert, not duplicate
      await queueReport(report); setQueuedCount((await getQueued()).length);
      alert("Report saved offline and queued for synchronization.");
    }
    setSaving(false);
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <form onSubmit={submit} className="bg-white border border-slate-200 rounded-lg p-5 space-y-4 shadow-sm">
          ...
          <div className="border border-slate-200 rounded-lg p-3 bg-slate-50">
            ...
            <div className="flex gap-2 mt-3">{!camOn ? <button type="button" onClick={startCam} className="px-3 py-1.5 rounded bg-emerald-600 text-white text-xs font-black">📷 Open Cam</button> : <button type="button" onClick={stopCam} className="px-3 py-1.5 rounded bg-slate-700 text-white text-xs font-black">■ Close Cam</button>}{camOn && <button type="button" onClick={capture} className="px-3 py-1.5 rounded bg-slate-900 text-white text-xs font-black">Capture</button>}</div>
