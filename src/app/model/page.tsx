    ...
    <div className="min-h-screen bg-slate-50 p-4">
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          {((live?.districts && live.districts.length)? live.districts : [{name:"Guwahati/Assam"},{name:"Itanagar/Arunachal"},{name:"Shillong/Meghalaya"},{name:"Imphal/Manipur"},{name:"Kohima/Nagaland"},{name:"Aizawl/Mizoram"},{name:"Agartala/Tripura"}]).slice(0,7).map((d:any)=><div key={d.name} className="bg-white border rounded p-2"><p className="text-[10px] font-bold truncate">{d.name}</p><p className="text-lg font-black">{d.rainfall !=null? d.rainfall+"mm" : "--"}</p><p className="text-[11px] capitalize">{d.severity || "--"}</p><p className="text-[10px] text-slate-500">{d.liveRisk !=null? d.liveRisk+"% risk" : "--"}</p></div>)}
